...
#' @param mats monthly list of transition matrices generated by fit_mc()
#' @export
#' @examples
#' mats <- fit_mc(x=sample(c('d', 'w', 'e'), size=720, replace=TRUE, prob=c(0.5, 0.3, 0.2)), months=rep(rep(seq(1, 12), each=30), times=2))
#' sim_mc(months=rep(1:12, each=30), initial='d', states=c('d', 'w', 'e'), mats=mats)
#'
sim_mc <- function(months, initial, states, mats) {
  n <- length(months)
  ...
  for (i in 2:n) {
    chain[i] <- sample(states, size=1, prob=mats[[months[i-1]]][chain[i-1], ])
  }
...
fit_mc <- function(x, months) {
  ...
  x_next <- lead(x)
  mats <- lapply(seq(1, 12), function(m) {
    idx <- which(months==m)
