...
gen_month_arma <- function(x.reg.yr, x.loc.mon, n.iter, n.year) {
  ...
  for (samp in 1:n.iter) {
    ...
    for (j in 1:n.year) {
      ...
      sim.mon[row.idx, , samp] <- data.matrix(filter(x.loc.mon,
                                                     YEAR==sampled.years[j, samp]) %>%
                                              mutate(YEAR_SIM=j,
                                                     YEAR_OBS=YEAR,
                                                     PRCP_OBS_YEAR=target.prcp) %>%
                                              select(YEAR_SIM, MONTH,
                                                     PRCP, TMAX, TMIN, TAVG,
                                                     YEAR_OBS, PRCP_OBS_YEAR))
    }
  }
...
#' @examples
#' sim.gen <- gen_month_arma(x.reg.yr=clim.reg.yr, x.loc.mon=clim.loc.mon, n.iter=50, n.year=nrow(clim.reg.yr))
#' trends_mon(sim.mon.list=sim.gen$sim.mon, temp.factor=2, prcp.factor=1.5)
#'
