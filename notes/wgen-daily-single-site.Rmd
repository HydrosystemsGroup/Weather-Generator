...
The daily weather generator is `wgen_daily()`. The following section will walk through this function, step-by-step.
...
n_year <- 10             # number of simulation years
...
```{r plot-annual-mc, fig.width=6, fig.height=3}
...
do.call(merge, z) %>% 
  ...
  geom_boxplot() + 
  stat_summary(fun.y=mean, geom='point', color='orangered', size=4) +
...
```{r plot-sim-day, fig.width=8, fig.height=4}
...
```{r plot-sim-day-yr-adj, fig.width=6, fig.height=3}
mutate(sim_da_unadj, WYEAR=wyear(DATE, start_month=start_month)) %>%
  ...
  ggplot(aes(factor(WYEAR), PRCP, color=SOURCE)) +
  geom_line()
```

```{r adj-sim-day-changes, warning=FALSE}
