...
thresh <- mc_state_threshold(prcp=clim.da[['PRCP']], months=clim.da[['MONTH']], 
...
```{r assign-state}
print(mc_sim)
sim$STATE <- mc_sim(months=month(sim$DATE), initial=sim[[1, 'STATE']], states=states, transitions=transitions)
```
...
for (i in 2:nrow(sim)) {
  m <- sim[i, 'MONTH']
  selected <- select_knn_day(wday=sim[[i, 'WDAY']], 
                             state=sim[[i, 'STATE']], 
...
df_equil <- lapply(seq(1, 12), function(m) {
  equil <- mc_equilibrium(transitions[[m]])
...
comp_state <- rbind(select(clim, DATE, MONTH, STATE) %>% mutate(SOURCE="hist"),
                    select(sim, DATE, MONTH, STATE) %>% mutate(SOURCE="sim"))
...
```{r sim-wet-dry}
sim_drier <- sim_mc_knn_day(n_year=length(unique(wyear(clim.da$DATE))), historical=clim, states=states, transitions=transitions_drier)
sim_wetter <- sim_mc_knn_day(n_year=length(unique(wyear(clim.da$DATE))), historical=clim, states=states, transitions=transitions_wetter)
```
