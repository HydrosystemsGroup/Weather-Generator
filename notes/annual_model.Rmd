...
clim.da <- climate_cities[['boston']] %>%
  mutate(WYEAR=wyear(DATE, start.month=10),
         MONTH=month(DATE))
...
```{r}
plot_wgen_mc_stats(obs=clim.wyr$PRCP, wgen_mc=wgen_mc)
```

# Annual k-Nearest Neighbors

```{r knn-year-function}
print(select_knn_year)
sim_years <- select_knn_year(sim_prcp=wgen_mc$x[1, 1], obs_prcp=clim.wyr$PRCP, obs_years=clim.wyr$WYEAR, n=100)
```
...
clim.wyr %>%
  ...
  geom_point() +
  geom_hline(yint=wgen_mc$x[1, 1], linetype=2, color='red') +
...
```{r}
data.frame(WYEAR=select_knn_year(sim_prcp=wgen_mc$x[1, 1], obs_prcp=clim.wyr$PRCP, obs_years=clim.wyr$WYEAR, n=100000)) %>%
  group_by(WYEAR) %>%
  summarise(N=n()) %>%
  left_join(select(clim.wyr, WYEAR, PRCP)) %>%
  mutate(DISTANCE=sqrt((wgen_mc$x[1, 1] - PRCP)^2)) %>%
...
```{r}
sim_day_1 <- wgen_historical_day(sampled_day, n_year = 1, start_month = 10)
all(unique(wyear(sim_day_1$SAMPLE_DATE)) %in% sim_years)
```

```{r}
select(sim_day_1, WDAY, PRCP, TMIN, TMAX, TEMP) %>%
...
sim_day_list <- mclapply(seq_along(wgen_mc$x[, 1]), function(i) {
  ...
  sim_years <- select_knn_year(sim_prcp=sim_prcp, obs_prcp=clim.wyr$PRCP, obs_years=clim.wyr$WYEAR, n=100)
  sampled_day <- lapply(sim_years, function(yr) {
    clim.da[which(clim.da$WYEAR==yr), ]
  })
  ...
  sim_day_1 <- wgen_historical_day(sampled_day, n_year = 1, start_month = 10)
  stopifnot(all(unique(wyear(sim_day_1$SAMPLE_DATE)) %in% sim_years))
  sim_day_1['SIM_YEAR'] <- i
  sim_day_1
}, mc.cores=6)
