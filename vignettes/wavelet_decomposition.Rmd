...
First, a wavelet decomposition is applied to the the annual precipitation timeseries. This function returns a 2-d array with each column corresponding to a single component. The first column is the noise component.
...
The row-wise sum of the wavelet decomposition output then equals the precipitation timeseries

```{r wavelet decomp compare}
cbind(wave_decomp, 'SUM'=apply(wave_decomp, 1, sum), 'PRECIP'=clim.wyr$PRCP)
```

A linear arima model is then fit to each column of `wave_decomp`.

```{r warm model arima}
arima.models <- apply(wave_decomp, 2, auto.arima, max.p=2, max.q=2, 
                                                  max.P=0, max.Q=0,
                                                  stationary=TRUE)
arima.models
```

The arima models can then be used to generate forecasts.

```{r plot_arima_comp1}
plot(forecast(arima.models[['COMPONENT 1']], h=nrow(clim.wyr)), 
     main="Low Frequency Component Forecast",
     xlab="Years")
```

And similarly for the noise component.

```{r plot_arima_noise}
plot(forecast(arima.models[['NOISE']], h=nrow(clim.wyr)), 
     main="Noise Component Forecast",
     xlab="Years")
```

## Non-WARM Model

If we didn't want to use a WARM model with the wavelet decomposition, then we'd just use an arime model.

```{r arima model}
arima.models.no.warm <- list(PRECIP=auto.arima(clim.wyr$PRCP, max.p=2, max.q=2,
                                               max.P=0, max.Q=0, stationary=TRUE))
arima.models.no.warm[["PRECIP"]]
```

But then we loose the low frequency signal as this only returns an intercept and no coefficients.
...
The predicted precip timeseries is shown:

```{r plot_sim_precip, eval=FALSE, echo=FALSE}
par(mfrow=c(1,1))
...
```{r plot_sim_precip_ggplot, eval=FALSE}
library(tidyr)
...
This figure summarises the spectrum power for each period, and compares the mean, standard deviation and skey of the simulated (blue) and observed (red) annual timeseries.

```{r plot_sim_spectrum, eval=FALSE}
par(mfrow=c(2,2))
x1 <- 1
x2 <- wave.n
